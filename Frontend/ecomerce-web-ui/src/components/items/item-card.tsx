...
export default function ItemCard(
    ...
    const [listCartItems, setListCartItems] = useLocalStorage<Items[]>('cartItems', [])

    const handleAddToCart = (item: Items) => {
        const existingItemIndex = listCartItems.findIndex((cartItem: Items) => cartItem.id === item.id);
        let newListCartItem = listCartItems
        if (existingItemIndex !== -1) {
            if (newListCartItem[existingItemIndex].quantity) {
                newListCartItem[existingItemIndex].quantity += 1
            } else {
                newListCartItem[existingItemIndex].quantity = 1
            }
        } else {
        ...
        setListCartItems(newListCartItem)
        ShowToast(`${item.name} has been added to your cart!`, "SUCCESS", 1000)
    }
    return (
            ...
            <div className="w-full">
                ...
                <div className="flex items-center gap-2 mb-3">
                    <span className="text-sm font-medium bg-gray-100 px-2 py-1 rounded-md text-gray-700">
                        {properties.quantity > 0 ? `In stock: ${properties.quantity}` : "Out of stock"}
                    </span>
                    {properties.quantity < 10 && properties.quantity > 0 && (
